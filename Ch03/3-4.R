...
#select - DF의 특정 컬럼 선택해서 새로운 DF 생성
df_math    <- df_exam %>% select(math)
df_english <- df_exam %>% select(english)
df_1       <- df_exam %>% select(math, science)
df_2       <- df_exam %>% select(everything())
...
#filter - DF의 특정 조건에 맞는 컬럼 선택해서 새로운 DF 생성
...
df_grade <- df_mean %>% mutate(total = math+english+science) %>% 
                        mutate(mean = total/3) %>% 
...
#inner_join - 두개 이상의 DF의 공통 컬럼으로 조인
